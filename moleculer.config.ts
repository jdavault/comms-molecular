...
import { config as configEnvironmentVars } from "dotenv-flow";
// @ts-ignore
import HealthMiddleware from "@r2d2bzh/moleculer-healthcheck-middleware";

configEnvironmentVars();

const transporter = (["production", "development"].includes(process.env.NODE_ENV)) ? "TRANSPORTER=nats://nats:4222" : null;
const cacher = (["production", "development"].includes(process.env.NODE_ENV)) ? "Redis" : null;
